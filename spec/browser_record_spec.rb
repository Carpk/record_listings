...
RSpec.describe "BrowserRecord" do
  ...
  it "should order list by gender" do
    ...
    expect(BrowserRecord.sort_by("gender").first).to eq(first_listed)
  end

  it "should order list by gender" do
    last_listed = "Feynman, Richard, Male, Blue, 1918-05-11"
    expect(BrowserRecord.sort_by("gender").last).to eq(last_listed)
  end

  it "should order list by birthdate" do
    first_listed = "Hopper, Grace, Female, Red, 1906-12-09"
    expect(BrowserRecord.sort_by("birthdate").first).to eq(first_listed)
  end

  it "should order list by birthdate" do
    last_listed = "Kaku, Michio, Male, Green, 1947-01-24"
    expect(BrowserRecord.sort_by("birthdate").last).to eq(last_listed)
  end

  it "should order list by lastname" do
    first_listed = "Feynman, Richard, Male, Blue, 1918-05-11"
    expect(BrowserRecord.sort_by("lastname").first).to eq(first_listed)
  end

  it "should order list by lastname" do
    last_listed = "Kaku, Michio, Male, Green, 1947-01-24"
    expect(BrowserRecord.sort_by("lastname").last).to eq(last_listed)
  end
